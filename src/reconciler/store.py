"""Route53 as etcd. (Reconciler copy — kept identical to the apiserver copy.)
...
knows the wire format; both the apiserver and the reconciler ship a copy.
"""
...
def _parse_txt_value(value: str) -> list[str]:
    ...
    # value, then split on the '" "' separator between chunks.
    if not value:
        return []
    v = value.strip()
    if v.startswith('"'):
...
def exists_marker(name: str) -> bool:
    resp = r53.list_resource_record_sets(
        HostedZoneId=HOSTED_ZONE_ID, StartRecordName=name, StartRecordType="TXT", MaxItems="1"
    )
    for rr in resp.get("ResourceRecordSets", []):
...
def list_kind(kind_plural: str, namespace: str | None = None) -> list[dict]:
    ...
    while True:
        ...
        if resp.get("IsTruncated"):
            kwargs.update(StartRecordName=resp["NextRecordName"], StartRecordType=resp["NextRecordType"])
        else:
            break
    return out


def upsert_alias(name: str, target_dns: str, target_zone: str) -> None:
    """Publish a Service/Ingress external address as an alias A record."""
    r53.change_resource_record_sets(
        HostedZoneId=HOSTED_ZONE_ID,
        ChangeBatch={"Changes": [{
            "Action": "UPSERT",
